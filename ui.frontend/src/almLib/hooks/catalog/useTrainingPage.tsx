...
import { getJobaidUrl, isJobaidContentTypeUrl, splitStringIntoArray } from "../../utils/catalog";
import {
  ADMIN_ENROLL,
  CERTIFICATION,
  COURSE,
...
export const useTrainingPage = (
  ...
  const { trainingInstance, isPreviewEnabled, isFlexLPValidationEnabled, isLoading, errorCode } =
    currentState;
  ...
  const navigateToHomePageHandler = (errorMessage: string) => {
    ...
    getALMObject().navigateToHomePage(3000);
  };
  ...
  useEffect(() => {
    if (trainingInstance?.id) {
      ...
      // Case - when other course url is copied and pasted
      const { currentPath } = getBreadcrumbPath();
      const currentPathId = splitStringIntoArray(currentPath, "::")[0];
      if (currentPathId && currentPathId !== trainingId) {
        clearBreadcrumbPathDetails();
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
