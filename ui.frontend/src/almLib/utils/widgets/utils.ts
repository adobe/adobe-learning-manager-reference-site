...
export function setWidgetAttributes(
  ...
  widget.layoutAttributes = widget.layoutAttributes || {};
  widget.layoutAttributes.cardsToShow = widget.attributes?.numCards || 1;
  const width = CARD_WIDTH * widget.layoutAttributes.cardsToShow;
  widget.layoutAttributes.width = `${width}px`;
  widget.layoutAttributes.widthInNumber = width;
  ...
  if (isMyLearningFullRowWidget) {
    const numberCount = getMyLearningCardsCount(widgets, maxCardsPossiblePerRow, index);
    widget.layoutAttributes.cardsToShow = numberCount || 1;
    const width = CARD_WIDTH * numberCount;
    widget.layoutAttributes.width = `${width}px`;
    widget.layoutAttributes.widthInNumber = width;
    widget.layoutAttributes.isFullRow = false;
  } else if (!isFlexiWidget) {
    widget.layoutAttributes.cardsToShow = maxCardsPossiblePerRow || 1;
    widget.layoutAttributes.isFullRow = true;
  }
...
export function getMyLearningCardsCount(
  ...
  widgets.forEach((widget: any, index: number) => {
    if (index != currentWidgetIndex) {
      totalCards += widget.attributes?.numCards || 1;
    }
  });

  if (totalCards < maxCardsPossiblePerRow) {
    return maxCardsPossiblePerRow - totalCards;
