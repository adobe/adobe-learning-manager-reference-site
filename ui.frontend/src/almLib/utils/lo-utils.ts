...
export function getTrainingLink(trainingId: string, accountId: string) {
  return (
    (getALMObject().getTrainingUrl && getALMObject().getTrainingUrl(accountId, trainingId)) ||
    getTrainingUrl(window.location.href)
  );
}

export function shouldDisableStart(
  coreContentModules: PrimeLearningObjectResource[],
  isModuleCrOrVc: boolean
): boolean {
  const crOrVcTime =
    coreContentModules?.length &&
    new Date(coreContentModules[0]?.resources?.[0]?.dateStart).getTime();
  if (!crOrVcTime && isModuleCrOrVc) {
    return true;
  }
  const currentTime = Date.now();
  const differenceInMinutes = (crOrVcTime - currentTime) / (1000 * 60);
  return isModuleCrOrVc && differenceInMinutes >= 15;
}