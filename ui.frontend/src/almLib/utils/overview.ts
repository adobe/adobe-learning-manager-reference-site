...
export const checkLoResourceForModuleLocking = (
  ...
  for (let index = 0; index < loResources.length; index++) {
    ...
    if (index > 0 && prevResource && loResourceType === CONTENT) {
      ...
      if (isNonBlockingChecklistModule(prevResource) || prevResGrade[0].hasPassed) {
        return false;
      }
      return true;
    }
...
export const checkIsTrainingLocked = (
  ...
  if (sections) {
    return checkIsTrainingLockedInsideSections(sections, training, subLOs);
  }
  // For Certifications
  return checkIsTrainingLockedForSubLOs(training, subLOs);
};
...
const checkIsTrainingLockedInsideSections = (
  ...
  if (loSectionIndex === 0) {
    ...
    return !isCurrentOrPreviousTrainingsCompleted(trainingEnrollment, previousLOs);
  }
  ...
  // For Section Index > 0 and LO Index > 0
  return checkIsTrainingLockedForSubLOs(training, parentSubLOs, sections);
};
...
const checkIsTrainingLockedForSubLOs = (
  ...
  // For subLoIndex > 0
  const previousLos = parentSubLOs.slice(0, subLoIndex);
  return !isCurrentOrPreviousTrainingsCompleted(training.enrollment, previousLos);
};
...
const isCurrentOrPreviousTrainingsCompleted = (
  ...
  return previousLos.every(lo => lo.enrollment && isTrainingCompleted(lo.enrollment));
};
