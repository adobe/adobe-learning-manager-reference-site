...
import { getALMObject, getWidgetConfig, getWindowObject } from "../../../utils/global";
...
const PrimeTrainingCardV2: React.FC<{
  ...
  const getSkillsOrPRLElement = (icon: JSX.Element, id: string, value: string) => {
    return (
      <>
        {icon}
        <span className={styles.skillName} id={id} data-automationid={id}>
          {value}
        </span>
      </>
    );
  ...
  const jobAidIconTemplate = () => {
    return isEnrolled
      ...
      : getJobAidDetails(enroll, "alm.overview.job.aid.add.from.list", ADD_BUTTON_SVG());
  };
        ...
        >
          <div className={styles.imageContainer} style={{ ...cardBgStyle }}>
            ...
            {training.duration ? (
              ...
              >
                {convertSecondsToHourAndMinsText(training.duration)}
              </div>
            ) : null}
