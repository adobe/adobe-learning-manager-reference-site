...
const PrimeTrainingPageMetaData: React.FC<{
  ...
  const completionDeadline = trainingInstance.completionDeadline || trainingInstance.enrollment?.completionDeadline || enrollment?.completionDeadline;
  ...
  const playerHandler = (newEnrollment?: PrimeLearningObjectInstanceEnrollment) => {
    ...
    if (isFlexLPOrContainsFlexLP) {
      ...
      const { id, moduleId, instanceId } = enrollViaModuleClick;
  ...
  function disableStartForCourse(course: PrimeLearningObject): boolean {
    ...
    if (coreContentOfTheCourse?.length > 1) {
      if (training?.isSubLoOrderEnforced) {
        ...
        if (
          (firstModule.resourceType === VIRTUAL_CLASSROOM || firstModule.resourceType === CLASSROOM) &&
  ...
  return (
    <section>
      ...
      !courseInstanceNotSelected
        ...
        : ""}
      {!showPreviewButton && !isCourseNotEnrollable && (
        <div className={styles.actionContainer}>
