...
import { convertSecondsToTimeText, modifyTimeDDMMYY } from "../../../utils/dateTime";
...
const PrimeTrainingPage = (props: any) => {
  ...
  const handleEmailFeedbackLaunch = (event: any) => {
    try {
  ...
  const getDuration = (
    ...
    return convertSecondsToTimeText(duration);
  };
  ...
  const processEnrollment = (token: string) => {
    const headers = getExtensionHeaders(token);
    try {
      enrollmentHandler({
        headers,
