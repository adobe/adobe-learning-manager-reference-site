...
class CommerceCustomHooks implements ICustomHooks {
  ...
  async getTrainings(filterState: CatalogFilterState, sort: string, search: string = "") {
    if (isUserLoggedIn()) {
  ...
  async loadMoreTrainings(
    ...
    if (isUserLoggedIn()) {
      return ALMCustomHooksInstance.loadMoreTrainings(filterState, sort, search, currentPage);
    }
    try {
      const filter = await getTransformedFilter(filterState);
