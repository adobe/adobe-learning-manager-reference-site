...
function getFilteredLocalizedData_lxpv<T>(localizedMetadata: T[], locale: string): T[] {
...
const PrimeModuleItem = (props: any) => {
  ...
  let preferredLocalizedData = getFilteredLocalizedData_lxpv(localizedMetadata, locale);
  if(!preferredLocalizedData) {
      preferredLocalizedData = getFilteredLocalizedData_lxpv(localizedMetadata, "en-US");
  }
  if(!preferredLocalizedData) {
  ...
  return (
    <>
      ...
      <div style={{ display: "flex" }}>{localizedData.name}</div>
