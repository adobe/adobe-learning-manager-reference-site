...
const PrimeCommunityPoll= (props: any) => {
    ...
    const updatePostStats = () => {
        ...
        if(pollStats.length > 0) {
            ...
            currentPollStats[parseInt(choiceSelectedIndex) - 1] = isNaN(currentPollStats[parseInt(choiceSelectedIndex) - 1]) ? 1 : parseInt(currentPollStats[parseInt(choiceSelectedIndex) - 1]) + 1;
    ...
    const postSubmitActions = (id?: any) => {
        if(id) {
            let selectedOptionValue = document.getElementById(props.post.id + "-input-" + id)?.getAttribute("value") || "";
    ...
    const getVotePercent = (value: any) => {
        ...
        return valueUptoTwoDecimalPlaces.endsWith(".00") ? valueUptoTwoDecimalPlaces.substring(0, valueUptoTwoDecimalPlaces.length - 3) : valueUptoTwoDecimalPlaces;
    }
            ...
            {pollOptions.map((item: any, index: any) => (
                <div className={styles.primeCommunityPollContainer} key={props.post.id + "-" + index}>
                    <div className={styles.primeCommunityPollButtonContainer}>
                        <input id={props.post.id + "-input-" + (index + 1)} name="pollOption" disabled={alreadyVoted} className={styles.primeCommunityPollButton} value={item.text} type="radio" onClick={() => {selectPollOption(item.text, index + 1)}}></input>
                    </div>
                    <div className={styles.primeCommunityPollOptionContainer}>
                        <div className={styles.primeCommunityPollOption}>
                            {item.text}
                        </div>
                        {alreadyVoted &&
                            <div id={props.post.id + "-fill-" + index}></div>
                        }
                    </div>
