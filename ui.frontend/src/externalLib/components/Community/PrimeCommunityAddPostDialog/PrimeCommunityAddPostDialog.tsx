...
import { uploadFile, cancelUploadFile } from "../../../utils/uploadUtils";
    ...
    <Dialog UNSAFE_className={styles.primeConfirmationDialog}>
      <Heading>
        <div className={styles.primeDialogHeaderContainer}>
          ...
          <button className={styles.primeDialogHeader} onClick={fileUploadClickHandler}>
            <div className={styles.primeDialogHeaderSvg}>
              {SOCIAL_UPLOAD_SVG()}
              <input className={styles.primeFileUploadInput} type="file" id={inputField} onChange={fileUploadHandler} accept={supportedFileTypes}></input>
            </div>
            <div className={styles.primeDialogHeaderText}>
              {formatMessage({id: "prime.community.upload.label",defaultMessage: "Upload"})}
            </div>
          </button>
        </div>      
      </Heading>
      <Content>
                  ...
                  {questionTypeSelected ?
                    <div className={styles.primeOption} onClick={questionButtonClickHandler}>              
                      ...
                      <div className={styles.primeOptionTextFilled}>
                        {formatMessage({id: "prime.community.markQuestion.label",defaultMessage: "Mark Question"})}
                      </div>
                    </div>
                    :
                    <div className={styles.primeOption} onClick={questionButtonClickHandler}>              
        ...
        {!textMode && fileUploadProgress !== 100 &&
          <div>
            <ProgressBar label={formatMessage({id: "prime.community.uploading.label", defaultMessage: "Uploading..."})} value={fileUploadProgress} />
          </div>
        }
        {!textMode && fileUploadProgress === 100 &&
          <div className={styles.primeStatus}>              
            <div className={styles.primeStatusText}>Uploaded: {state.fileUpload.fileName}</div>
            <button className={styles.primeStatusSvg} title={formatMessage({id: "prime.community.removeUpload.label", defaultMessage: "Remove upload"})} onClick={cancelClickHandler}>
              {SOCIAL_CANCEL_SVG()}
            </button>
          </div>
        }
      ...
      <ButtonGroup>
        <Button variant="secondary" onPress={(close) => {closeDialogHandler(close)}}>
          {formatMessage({id: "prime.community.cancel.label", defaultMessage: "Cancel"})}
        </Button>
        {saveEnabled ? 
          <Button variant="cta" onPress={(close) => {savePostHandler(close)}}>
            {formatMessage({id: "prime.community.post.label", defaultMessage: "Post"})}
          </Button>
          :
