...
export function getPreferredLocalizedMetadata<T>(localizedMetadata: T[], locale: string): T {
    ...
    return (
        ...
        localizedMetadata.find(
            (item: any) => item.locale === ENGLISH_LOCALE
        )! ||
        localizedMetadata[0]
    );
}
...
export function GetTranslationReplaced(key: string, repVal: string, replaceAccountTerminology = false): string {
    let val = _translations[key];
    if (replaceAccountTerminology) {
        val = ReplaceAccountTerminology(val);
    }
...
export function interpolateTemplateAndMap(template: string, params: Record<string, string | number | boolean>): any {
    const names = Object.keys(params);
    const vals = Object.values(params);
    return new Function(...names, `return \`${template}\`;`)(...vals);
}

export function GetTranslationsReplaced(key: string, paramMap: Record<string, string | number | boolean>, replaceAccountTerminology = false): string {
    let val = _translations[key];
    if (replaceAccountTerminology) {
        val = ReplaceAccountTerminology(val);
    }
...
export function ReplaceAccountTerminology(translation: string): string {
    ...
    if (pluralTokens) {
        pluralTokens.forEach(function (token) {
            translation = translation.replace(token, accountTerminologyMap[token.replace(pluralPrefix, "")]?.pluralName_lxpv);
        });
    }
    const singularTokens = translation.match(singularRegex);
    if (singularTokens) {
        singularTokens.forEach(function (token) {
            translation = translation.replace(token, accountTerminologyMap[token.replace(singularPrefix, "")]?.name_lxpv);
        });
    }
    return translation;
}

export function SetupAccountTerminologies(accountTerminologies: PrimeAccountTerminology[]): void {
