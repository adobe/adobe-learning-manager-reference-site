...
(function ($) {
  const CREATE_PULLDOWN_SELECTOR = ".alm-create-pulldown";
  const CREATE_CONFIG_SELECTOR = ".alm-create-config";
  const CREATE_FOLDER_SELECTOR = ".alm-create-folder";
  ...
  function applyRenderConditions(activeItem) {
    const activeItemMeta = activeItem.querySelector(
      ".foundation-collection-quickactions"
    );
    let actionRels;
    if (activeItemMeta) {
      actionRels =
        activeItemMeta.dataset["foundationCollectionQuickactionsRel"];
    }
    ...
    if (
      actionRels &&
      createPullDown &&
      actionRels.indexOf(CREATE_PULLDOWN_REL) >= 0
    ) {
      show(createPullDown);
    }
    ...
    if (
      actionRels &&
      createFolderButton &&
      actionRels.indexOf(CREATE_FOLDER_REL) >= 0
    ) {
    ...
    if (
      actionRels &&
      createConfigButton &&
      actionRels.indexOf(CREATE_CONFIG_REL) >= 0
    ) {
  ...
  function init() {
    ...
    const activeItem = document.querySelector(
      ".cq-confadmin-admin-childpages.foundation-collection"
    );
    if (activeItem) {
