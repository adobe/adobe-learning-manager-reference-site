...
(function (window, document, Granite, $) {
  ...
  const isPrimeUserLoggedIn = () => (usageMap[CURRENT_USAGE_TYPE].isLoggedIn() === "" ? false : true);

  const handleLogIn = (queryParams) => {
    usageMap[CURRENT_USAGE_TYPE].handleLogIn();
  };

  const handleLogOut = () => {
    usageMap[COMMERCE_USAGE_TYPE].handleLogOut();
  };

  const handleRegister = () => {
    usageMap[COMMERCE_USAGE_TYPE].handleRegister();
  };
