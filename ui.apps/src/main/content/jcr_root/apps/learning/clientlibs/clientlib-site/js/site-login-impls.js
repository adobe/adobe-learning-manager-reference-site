(function (window) {
    ...
    const PROXY_ALM_LOGIN_URL = "/almLogin";
    const PROXY_ALM_COMMERCE_LOGIN_URL = "/almCommerceLogin";
    const PROXY_ALM_LOGOUT_URL = "/almLogout";
    ...
    class AlmLogin {
        ...
        async isLoggedIn() {
            ...
            try {
                const isLoggedIn = await this.makeAjaxRequest(PROXY_ALM_LOGIN_URL, "GET", {});
    ...
    class AlmPrimeLogin extends AlmLogin {
        ...
        async isLoggedIn() {
            let isLoggedIn;
            try {
                isLoggedIn = await super.isLoggedIn();
            } catch(error) {
                console.error("Error in AlmPrimeLogin isLoggedIn:", error);
                isLoggedIn = false;
            }
            return isLoggedIn;
        }
    ...
    class AlmEsLogin extends AlmLogin {
        ...
        handleRegister() {
            ...
            return `${almBaseURL}/oauth/o/authorize?client_id=${clientId}&redirect_uri=${window.location.href}&state=${ES_REGISTER_STATE}&scope=learner:read,learner:write&response_type=CODE&client_identifier=aemsite&logoutAfterAuthorize=true&loginUrl=${encodeURIComponent(almRegisterUrl)}`;
        }
    ...
    class AlmCommerceLogin extends AlmLogin {
        ...
        async isCommerceLoggedIn() {
            if (window.ALM.isProxyEnabled()) {
                try {
                    let isLoggedIn = await super.makeAjaxRequest(PROXY_ALM_COMMERCE_LOGIN_URL, "GET", {});
                    console.log(isLoggedIn);
                    isLoggedIn = true;
                    return isLoggedIn;
                } catch (error) {
        ...
        async handlePageLoad() {
            ...
            try {
                isCommerceLoggedIn = await this.isCommerceLoggedIn();
                console.log(isCommerceLoggedIn);
                isCommerceLoggedIn = true;
            } catch (error) {
                isCommerceLoggedIn = false;
            }

            if (!isLoggedIn || (!window.ALM.isAuthor() && !isCommerceLoggedIn)) {
                await super.cleanUpUserData();
            }
