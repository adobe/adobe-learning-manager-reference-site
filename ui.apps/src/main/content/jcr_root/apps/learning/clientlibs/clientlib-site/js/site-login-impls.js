(function (window) {
    ...
    class AlmLogin {
        ...
        isLoggedIn() {
            ...
            // try {
            //     const isLoggedIn = await this.makeAjaxRequest(PROXY_ALM_LOGIN_URL, "POST", {});
    ...
    class AlmPrimeLogin extends AlmLogin {
        ...
        async handlePageLoad() {
            let isLoggedIn;
            try {
                isLoggedIn = await this.makeAjaxRequest(PROXY_ALM_LOGIN_URL, "POST", {});
                console.log(isLoggedIn);
                isLoggedIn = true;
            } catch (error) {
                console.error("Error in checking logged in");
    ...
    class AlmCommerceLogin extends AlmLogin {
        ...
        async handlePageLoad() {
            const isALMLoggedIn = this.isLoggedIn();

            let isCommerceLoggedIn;
            try {
                isCommerceLoggedIn = await super.makeAjaxRequest(PROXY_ALM_COMMERCE_LOGIN_URL, "POST", {});
                console.log("isCommerceLoggedIn : " + isCommerceLoggedIn);
