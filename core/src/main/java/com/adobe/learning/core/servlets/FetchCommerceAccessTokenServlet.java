...
@Component(
    ...
    property = {
      ...
      "sling.servlet.resourceTypes=" + FetchCommerceAccessTokenServlet.RESOURCE_TYPE,
      "sling.servlet.selectors=" + "commerceToken",
...
public class FetchCommerceAccessTokenServlet extends SlingAllMethodsServlet {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(FetchCommerceAccessTokenServlet.class);

  private static final String COMMERCE_EMAIL_QUERY =
      "{\"query\":\"query {\\n customer {\\n firstname \\n lastname \\n email\\n}\\n}\"}";

  private static final String ALM_CREATE_USER_BODY =
      "{\"data\": { \"type\": \"user\", \"attributes\": {\"email\":\"{email}\",\"name\": \"{name}\",\"userType\":\"INTERNAL\"}}}";
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) {
    try {
      ...
      if (currentPage == null) {
        ...
        response.sendError(
            HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Unable to get current page");
        return;
      }
      ...
      Integer customerTokenLifetime =
          Integer.valueOf(
              jsonConfigs.get(Constants.Config.COMMERCE_CUSTOMER_TOKEN_LIFETIME).getAsString());
      ...
      if (customer != null) {
        ...
        if (containValidAccessToken(accessTokenResp)) {
          ...
          setAccessTokenCookie(
              request,
              response,
              accessTokenWithExpiry.getLeft(),
              accessTokenWithExpiry.getRight(),
              customerToken,
          ...
          if (success) {
            ...
            LOGGER.error(
                "CPPrime::FetchCommerceAccessTokenServlet:: Exception in creating ALM User. Response- {}",
                accessTokenResp);
            response.sendError(
                HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Unable to create ALM user");
            return;
          }
        }
      }
      ...
      LOGGER.error(
          "CPPrime::FetchCommerceAccessTokenServlet:: Exception in fetching access_token.");
      response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);

    } catch (Exception e) {
      LOGGER.error(
          "CPPrime::FetchCommerceAccessTokenServlet Exception while fetching access_token", e);
    }
  }
  ...
  private boolean createALMUser(
      ...
      String firstName,
      String lastName) {
    Pair<String, Integer> accessToken =
    ...
    if (accessToken != null) {
      ...
      try (CloseableHttpClient httpClient = RequestUtils.getClient(clientBuilderFactory);
        ...
        if (HttpStatus.SC_OK == response.getStatusLine().getStatusCode()) return true;

        LOGGER.error(
            "CPPrime::FetchCommerceAccessTokenServlet::createALMUser Exception while creating ALM User. Response {}",
  ...
  private boolean userNotPresent(String accessTokenResp) {
    ...
    if (respObj != null) {
      JsonObject sourceObj =
          respObj.get("source") != null ? respObj.get("source").getAsJsonObject() : null;
      if (sourceObj != null) {
    ...
    try {
      ...
      try (CloseableHttpClient httpClient = RequestUtils.getClient(clientBuilderFactory);
        ...
        if (StringUtils.isNotBlank(responseStr)
          ...
          String customerResp =
              String.valueOf(
                  gson.fromJson(responseStr, JsonObject.class)
                      .get("data")
                      .getAsJsonObject()
                      .get("customer"));
        ...
        LOGGER.error(
            "CPPrime::FetchCommerceAccessTokenServlet::getCustomerFromCommerce Exception in http call while fetching customer from commerce",
            e);
      }

    } catch (UnsupportedEncodingException uee) {
      LOGGER.error(
          "CPPrime::FetchCommerceAccessTokenServlet::getCustomerFromCommerce Exception in character encoding while fetching email from commerce",
          uee);
    }
    return null;
  }
  ...
  private void setAccessTokenCookie(
      ...
      String accessToken,
      Integer cpTokenTimeout,
      String commerceToken,
      Integer commerceTimeout,
      Boolean storeTokenInCookie) {
    final Cookie tokenCookie = new Cookie(Constants.Config.ACCESS_TOKEN_COOKIE_NAME, accessToken);
    final Cookie customerTokenCookie =
        new Cookie(Constants.Config.CUSTOMER_TOKEN_COOKIE_NAME, commerceToken);
    ...
    if (!storeTokenInCookie) {
      customerTokenCookie.setHttpOnly(true);
    }
