...
public class GlobalConfigurationServiceImpl implements GlobalConfigurationService {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(GlobalConfigurationServiceImpl.class);
  ...
  public JsonObject getAdminConfigs(Page currentPage) {
    ...
    try (ResourceResolver serviceResolver = resolverFactory.getServiceResourceResolver(authInfo)) {
      ...
      if (resource == null) {
        LOGGER.error(
            "Service user permissions of {} are not sufficient to view resource at {}",
        ...
        adminConfigs =
            (cfgBuilder != null)
                ? cfgBuilder.name(Constants.Config.ALM_CONFIGURATION_NAME).asValueMap()
                : null;
        if (adminConfigs == null || adminConfigs.size() == 0) {
          InheritanceValueMap inheritedVM = new HierarchyNodeInheritanceValueMap(resource);
          String cpConfPath =
              inheritedVM.getInherited(Constants.Config.CONF_PROP_NAME, String.class);
          if (cpConfPath != null) {
