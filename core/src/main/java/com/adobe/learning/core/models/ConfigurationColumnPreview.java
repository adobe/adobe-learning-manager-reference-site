...
public class ConfigurationColumnPreview {
  ...
  protected void initModel() {
    ...
    final SlingScriptHelper sling =
        ((SlingBindings) request.getAttribute(SlingBindings.class.getName())).getSling();
    ...
    isFolder =
        itemResource.isResourceType(JcrConstants.NT_FOLDER)
            || itemResource.isResourceType(JcrResourceConstants.NT_SLING_FOLDER)
            || itemResource.isResourceType(JcrResourceConstants.NT_SLING_ORDERED_FOLDER);
    ...
    modifiedTime = properties.get(NameConstants.PN_PAGE_LAST_MOD, Calendar.class);
